...
def _permutation_test_score(estimator, X, y, groups=None, cv='warn',
    ...
    ## Changed arguments in cross_val_predict from positional to keyword arguments
    y_pred = cross_val_predict(estimator=estimator, X=X, y=y, 
                               groups=groups, cv=cv, n_jobs=n_jobs, 
                               verbose=verbose, params=fit_params, 
                               pre_dispatch=pre_dispatch, method=method)
